...
Rozpoczniemy od losowego podziału danych na zbiór uczący (80\%) i testowy (20\%).

<<ustalenie.ziarna, echo=FALSE>>=
set.seed(14052020)
@

<<podzial.uczacy.testowy>>=
n <- nrow(Glass)
...
<<podstawa_knn>>=
model.knn <- ipredknn(Type ~ ., data=zbior.uczacy)
...
blad.klasyfikacji.knn <-  1 - poprawne.wyniki / nrow(zbior.testowy)
@

<<podstawa_drzewo>>=
model.drzewo <- rpart(Type ~ ., data=zbior.uczacy)

predykcje <- predict(model.drzewo, zbior.testowy, type="class")
macierz.konfuzji <- table(predykcje, zbior.testowy$Type)

poprawne.wyniki <- sum(diag(macierz.konfuzji))
blad.klasyfikacji.drzewo <-  1 - poprawne.wyniki / nrow(zbior.testowy)

rpart.plot(model.drzewo)
@

<<podstawa_nb>>=
model.nb <- naiveBayes(Type ~ ., data=zbior.uczacy)

predykcje <- predict(model.nb, zbior.testowy, type="class")
macierz.konfuzji <- table(predykcje, zbior.testowy$Type)

poprawne.wyniki <- sum(diag(macierz.konfuzji))
blad.klasyfikacji.nb <-  1 - poprawne.wyniki / nrow(zbior.testowy)
@

<<echo=FALSE>>=
wyniki <- data.frame(
  blad.klasyfikacji.knn,
  blad.klasyfikacji.drzewo,
  blad.klasyfikacji.nb,
  row.names = "błąd klasyfikacji")
names(wyniki) <- c("knn", "drzewo", "nb")
kable(
  wyniki,
...
Do dalszych estymacji błędu klasyfikacji, wykorzystamy następujące funkcje pomocnicze
odpowiadającą za uczenie modelu oraz estymację błędów.

<<>>=
mypredict <- function(model, newdata) predict(model, newdata, type="class")
my.errorest <- function(formula, data, model){
  cv.10 <- errorest(
  ...
  return(list(
    cv.5=cv.5, cv.10=cv.10, leave.one.out=leave.one.out,
    bootstrap=bootstrap, err632plus=err632plus
  ))
}
@
...
wyniki <- data.frame(
  row.names=c(
    "CV5", "CV10", "leave-one-out",
    "bootstrap (25 replications)",
    "632+ (25 replications)"
  )
)
wyniki$knn <- my.errorest(Type ~ ., Glass, ipredknn)
wyniki$drzewo <- my.errorest(Type ~ ., Glass, rpart)
wyniki$nb <- my.errorest(Type ~ ., Glass, naiveBayes)

wyniki
@

<<surowe.porownanie, echo=FALSE>>=
wyniki <- data.frame(
  row.names=c(
    "CV5", "CV10", "leave-one-out",
    "bootstrap (25 replications)",

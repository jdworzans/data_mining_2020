...
Na podstawie wykresu \ref{fig:klasy} widzimy, że obiekty są uporządkowane ze względu na przynależność do danej klasy:
...
Do konstrukcji drugiego modelu regresji wykorzystujemy zbiór z wyjściowymi cechami oraz zmiennymi \textit{Petal.Length}$^2$ oraz \textit{Petal.Width}$*$\textit{Sepal.Length}. 

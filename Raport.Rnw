...
Z uzyskanych wyników, dowiedzieliśmy się, że większość klientów firmy, to klienci niekorzystający z dodatkowych usług, co jednak może być spowodowane nieadekwatną ofertą.

Na podstawie przeprowadzonych analiz, możemy zauważyć, że odchodzenie klientów jest spodowowane ofertą, która jest dostosowana wyłącznie do podstawowych potrzeb. Widzimy, że dodatkowe usługi, takie jak plan międzynarodowy, nie są korzystne dla klientów i może to być przyczyną odchodzenia klientów z firmy. Ponadto, wygląda na to, że oferta nie jest dostosowana do osób, które dzwonią więcej niż pozostali. Przyczyny można również dopatrywać się w obsłudze klienta.

Stąd, możemy wyszczególnić główne 3 problemy:

\begin{itemize}
\item Oferta nie jest dostosowana do osób, które dużo rozmawiają.
...
Aby podnieść lojalność klientów, można dostosować ofertę międzynarodową do potrzeb klientów, przygotować dodatkową ofertę dla klientów, którzy rozmawiają więcej. Ponadto, można skontrolować biuro obsługi klienta, aby sprawdzić, czy przyczyną odejść są zaniedbania pracowników, czy też inne problemy.
